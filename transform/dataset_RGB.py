...
class DataLoaderTrain(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # resize img
        # if hh != ww:
        #     inp_img = transform.Resize((512, 512))(inp_img)
        #     tar_img = transform.Resize((512, 512))(tar_img)
...
class DataLoaderTest_(Dataset):
    def __init__(self, rgb_dir, target_transform=None):
        ...
        clean_files = sorted(os.listdir(os.path.join(rgb_dir, 'input')))
        noisy_files = sorted(os.listdir(os.path.join(rgb_dir, 'target')))

        self.clean_filenames = [os.path.join(rgb_dir, 'input', x) for x in clean_files if is_image_file(x)]
        self.noisy_filenames = [os.path.join(rgb_dir, 'target', x) for x in noisy_files if is_image_file(x)]
